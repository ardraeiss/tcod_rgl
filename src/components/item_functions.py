...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        results.append({
            'consumed': True,
            'target': target,
            'message': Message('A lighting bolt strikes the {0} with a loud thunder! The damage is {1}'.
...
def cast_fireball(*args, **kwargs):
    ...
    for entity in entities:
        if entity.distance(target_x, target_y) <= radius and entity.fighter:
            results.append({
                'message': Message('The {0} gets burned for {1} hit points.'.format(entity.name, damage),
                                   tcod.orange)
            })
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            results.append({
                'consumed': True,
                'message': Message('The eyes of the {0} look vacant, as he starts to stumble around!'.
                                   format(entity.name),
                                   tcod.light_green)
            })
        ...
        results.append({
            'consumed': False,
            'message': Message('There is no targetable enemy at that location.',
                               tcod.yellow)
        })
